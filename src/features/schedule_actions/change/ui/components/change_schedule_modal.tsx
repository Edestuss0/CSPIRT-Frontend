...
import type { ScheduleChangeFormValues } from "../../model/schedule_change_usecase";
import type {ScheduleLessonType} from "../../../../../shared/entities/schedule/types/schedule_types.ts";
import {useChangeScheduleStore} from "../../store/schedule_change_store.ts";
import { createPortal } from "react-dom";
import {ConfirmModal} from "../../../../../shared/ui/modals/confirm_modal.tsx";
import {useClassDashboardStore} from "../../../../class_dashboard/store/class_dashboard_store.ts";
...
export function ChangeScheduleLessonModal({isOpen, onClose, lesson, onChanged, type,}: ChangeScheduleLessonModalProps) {
    const error = useChangeScheduleStore((state) => state.error);
    const status = useChangeScheduleStore((state) => state.status);
    const teachers = useChangeScheduleStore((state) => state.teachers);
    const changeSchedule = useChangeScheduleStore((state) => state.changeSchedule);
    const getTeachers = useChangeScheduleStore((state) => state.getTeachers);
    const [isDeleteLessonModalOpen, setIsDeleteLessonModalOpen] = useState(false);
    const deleteSchedule = useClassDashboardStore((state) => state.deleteSchedule)
        ...
        <div className="modal-backdrop" onMouseDown={onClose}>
            ...
            >
                ...
                <ConfirmModal
                    ...
                    isDanger={true}
                    title={"Отменить урок"}
                    content={`Отменить проведение урока?`}
