...
import type {addUserFormType, UserType} from "../../../shared/entities/user/types/user_types.ts";
...
import {AddUserUseCase} from "../models/add_user/add_user_usecase.ts";
...
interface State {
    ...
    addUser: (dto: addUserFormType) => Promise<void>;
...
export const useDashboardStore = create<State>()((set) => ({
    ...
    addUser: async (dto: addUserFormType) => {
        set({status: "loading"});
        
        try {
            await AddUserUseCase(dto);
